...
async def stream_boosting_frames(ws: WebSocket, algorithm: str, params: dict, dataset_name: str, resolution: int):
    X, y = generate_dataset(dataset_name, n_samples=300, noise=0.5)
...
async def stream_gradient_descent_frames(ws: WebSocket, params: dict, dataset_name: str, resolution: int):
...
async def stream_tree_growth_frames(ws: WebSocket, params: dict, dataset_name: str, resolution: int):
...
async def stream_mlp_frames(ws: WebSocket, params: dict, dataset_name: str, resolution: int):
    ...
    try:
        ...
        if stream_fn:
            if stream_fn is stream_boosting_frames:
                await stream_fn(websocket, algorithm, params, dataset_name, resolution)
            else:
                await stream_fn(websocket, params, dataset_name, resolution)
        else:
