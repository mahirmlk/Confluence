  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex items-center gap-1.5 p-2 md:p-3 border-b border-border bg-card/50 flex-wrap">
        ...
        {/* Mobile-only: expose desktop-hidden analysis actions via More menu */}
        <button
          onClick={() => setShowMore((v) => !v)}
          aria-expanded={showMore}
          className="sm:hidden px-2.5 py-1.5 rounded-md border border-border text-foreground text-[10px] font-medium hover:bg-accent transition-colors min-h-[36px]"
        >
          {showMore ? "Less" : "More"}
        </button>
      ...
      {/* Mobile-only overflow actions — same callbacks as desktop hidden buttons */}
      {showMore && (
        <div className="sm:hidden flex flex-wrap gap-1.5 px-2 pb-2 border-b border-border bg-card/50">
          {family === "classification" && (
            <>
              <button onClick={() => { fetchCV(); setShowMore(false); }} className="px-3 py-2 rounded-md border border-border text-foreground text-[11px] font-medium hover:bg-accent min-h-[40px]">Cross-Val</button>
              <button onClick={() => { fetchCoef(); setShowMore(false); }} className="px-3 py-2 rounded-md border border-border text-foreground text-[11px] font-medium hover:bg-accent min-h-[40px]">Coefficients</button>
              <button onClick={() => { fetchLC(); setShowMore(false); }} className="px-3 py-2 rounded-md border border-border text-foreground text-[11px] font-medium hover:bg-accent min-h-[40px]">Learn Curve</button>
              {algorithm === "decision-tree" && (
                <button onClick={() => { setActivePanel("tree-build"); setShowMore(false); }} className="px-3 py-2 rounded-md border border-border text-foreground text-[11px] font-medium hover:bg-accent min-h-[40px]">Tree</button>
              )}
              <button onClick={() => { setShowWrongPredictions(true); setShowMore(false); }} className="px-3 py-2 rounded-md border border-border text-foreground text-[11px] font-medium hover:bg-accent min-h-[40px]">Wrong Preds</button>
            </>
          )}
          {family === "regression" && (
            <>
              <button onClick={() => { fetchRegCV(); setShowMore(false); }} className="px-3 py-2 rounded-md border border-border text-foreground text-[11px] font-medium hover:bg-accent min-h-[40px]">Cross-Val</button>
              <button onClick={() => { fetchRegLC(); setShowMore(false); }} className="px-3 py-2 rounded-md border border-border text-foreground text-[11px] font-medium hover:bg-accent min-h-[40px]">Learn Curve</button>
            </>
          )}
          {family === "clustering" && (
            <button onClick={() => { fetchElbow(); setShowMore(false); }} className="px-3 py-2 rounded-md border border-border text-foreground text-[11px] font-medium hover:bg-accent min-h-[40px]">Elbow</button>
          )}
        </div>
      )}
      ...
      <div className="flex flex-1 overflow-hidden flex-col md:flex-row">
        ...
        <div
          ...
          className="flex-1 flex items-center justify-center p-3 max-md:p-2 md:p-6 relative overflow-hidden min-h-0 mobile-canvas-wrap"
        ...
        >
          ...
          {activePanel && (
            <div className="absolute top-2 left-2 md:top-4 md:left-4 w-[calc(100%-16px)] md:w-72 bg-card border border-border rounded-lg p-3 md:p-4 max-h-[70vh] md:max-h-[80vh] overflow-y-auto z-20 shadow-lg max-md:mobile-sheet">
              <div className="max-md:mx-auto max-md:mb-2 max-md:h-1 max-md:w-10 max-md:rounded-full max-md:bg-border md:hidden" aria-hidden="true" />
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => setActivePanel(null)} className="text-xs text-muted-foreground hover:text-foreground max-md:min-w-[44px] max-md:min-h-[44px] max-md:flex max-md:items-center max-md:justify-center">x</button>
              </div>
...
function AppContent() {
  ...
  const { width: appWinW } = useWindowSize();
  const streamSize = Math.max(280, Math.min(600, (appWinW || 360) - 32));
  useUrlState();

  const tabButtons = (
    <div className="flex flex-wrap max-md:flex-nowrap max-md:overflow-x-auto max-md:mobile-snap-x gap-0.5 mb-4 md:mb-6 border border-border rounded-lg p-0.5">
      {(["explore", "compare", "taxonomy", "stream", "playground", "tools", "help"] as Tab[]).map((t) => (
        <button key={t} onClick={() => { setTab(t); setSidebarOpen(false); }} className={`flex-1 max-md:flex-none max-md:min-w-[78px] max-md:min-h-[40px] px-1.5 md:px-2 py-1.5 rounded text-[10px] max-md:text-[11px] md:text-xs font-medium transition-colors ${tab === t ? "bg-primary text-primary-foreground" : "text-foreground hover:bg-accent"}`}>
    ...
    <div className="flex flex-col h-screen bg-background">
      ...
      <div className="flex flex-1 overflow-hidden flex-col md:flex-row">
        ...
        <button
          ...
          className="md:hidden fixed bottom-[calc(1rem+env(safe-area-inset-bottom))] left-4 z-50 w-11 h-11 rounded-full bg-primary text-primary-foreground shadow-lg flex items-center justify-center"
        ...
        <aside className={`
          ...
          w-80 md:w-80 max-md:w-[86vw] max-md:max-w-[320px]
          border-r border-border bg-card p-4 max-md:pb-[calc(1rem+env(safe-area-inset-bottom))] overflow-y-auto flex-shrink-0
        ...
        <main className="flex-1 overflow-auto bg-background">
          ...
          {tab === "stream" && (
            <div className="flex items-center justify-center p-4 max-md:p-3 md:p-8">
              <div className="w-full max-w-[600px]">
                <StreamingViz algorithm={algorithm} datasetName={datasetName} hyperparameters={hyperparameters} resolution={resolution} noise={noise} nSamples={nSamples} width={streamSize} height={streamSize} />
              </div>
            </div>
          )}
          {tab === "playground" && (
